...
class SearchEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = PassantenfrequenzStadtStgallenSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set PASSANTENFREQUENZ_STADT_STGALLEN_TEST_SEARCH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def search_basic_setup(extra)
  ...
  entid_env_raw = ENV["PASSANTENFREQUENZ_STADT_STGALLEN_TEST_SEARCH_ENTID"]
  ...
  if env["PASSANTENFREQUENZ_STADT_STGALLEN_TEST_LIVE"] == "TRUE"
    ...
    client = PassantenfrequenzStadtStgallenSDK.new(Helpers.to_map(merged_opts))
  end
