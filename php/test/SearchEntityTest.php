...
class SearchEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = PassantenfrequenzStadtStgallenSDK::test($seed, null);
        $seen = iterator_to_array($base->Search(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = PassantenfrequenzStadtStgallenConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = PassantenfrequenzStadtStgallenSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PASSANTENFREQUENZ_STADT_STGALLEN_TEST_SEARCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function search_basic_setup($extra)
{
    ...
    $client = PassantenfrequenzStadtStgallenSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("PASSANTENFREQUENZ_STADT_STGALLEN_TEST_SEARCH_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["PASSANTENFREQUENZ_STADT_STGALLEN_TEST_SEARCH_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["PASSANTENFREQUENZ_STADT_STGALLEN_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new PassantenfrequenzStadtStgallenSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["PASSANTENFREQUENZ_STADT_STGALLEN_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
