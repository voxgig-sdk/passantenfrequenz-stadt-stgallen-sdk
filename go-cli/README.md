# passantenfrequenz-stadt-stgallen-cli

boru-driven command-line client **and** interactive REPL for the PassantenfrequenzStadtStgallen
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/passantenfrequenz-stadt-stgallen-cli)
make build

# 2. See usage (words, entities, env vars)
./passantenfrequenz-stadt-stgallen-cli --help

# 3. Provide credentials once, via the environment
export PASSANTENFREQUENZ_STADT_STGALLEN_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./passantenfrequenz-stadt-stgallen-cli list search

# 5. Override the API base URL for a single call
PASSANTENFREQUENZ_STADT_STGALLEN_BASE=https://api.example.com ./passantenfrequenz-stadt-stgallen-cli list search

# 6. No arguments -> interactive REPL
./passantenfrequenz-stadt-stgallen-cli
passantenfrequenz-stadt-stgallen> list search
passantenfrequenz-stadt-stgallen> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/passantenfrequenz-stadt-stgallen-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/passantenfrequenz-stadt-stgallen-cli list search
   ```
...
Run with no arguments to open a REPL (prompt `passantenfrequenz-stadt-stgallen>`). Each line is
evaluated as its own boru expression:

```text
$ ./passantenfrequenz-stadt-stgallen-cli
passantenfrequenz-stadt-stgallen> list search
passantenfrequenz-stadt-stgallen> /help
passantenfrequenz-stadt-stgallen> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/passantenfrequenz-stadt-stgallen-cli`. |
...
(`./passantenfrequenz-stadt-stgallen-cli <expr>`) and interactively (the REPL), and expressions compose the
