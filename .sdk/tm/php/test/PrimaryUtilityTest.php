...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ...
        $ctx = new PassantenfrequenzStadtStgallenContext($ctxmap, null);
        ...
        if (isset($ctxmap['spec']) && is_array($ctxmap['spec'])) {
            $ctx->spec = new PassantenfrequenzStadtStgallenSpec($ctxmap['spec']);
        }

        // Handle result from JSON map
        if (isset($ctxmap['result']) && is_array($ctxmap['result'])) {
            $ctx->result = new PassantenfrequenzStadtStgallenResult($ctxmap['result']);
            if (isset($ctxmap['result']['err']) && is_array($ctxmap['result']['err'])) {
                $msg = $ctxmap['result']['err']['message'] ?? '';
                $ctx->result->err = new PassantenfrequenzStadtStgallenError('', $msg);
            }
        }

        // Handle response from JSON map
        if (isset($ctxmap['response']) && is_array($ctxmap['response'])) {
            $ctx->response = new PassantenfrequenzStadtStgallenResponse($ctxmap['response']);
    ...
    private static function fixctx(PassantenfrequenzStadtStgallenContext $ctx, PassantenfrequenzStadtStgallenSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?PassantenfrequenzStadtStgallenError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new PassantenfrequenzStadtStgallenError($code, $msg);
    }
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = PassantenfrequenzStadtStgallenSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new PassantenfrequenzStadtStgallenOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = PassantenfrequenzStadtStgallenSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof PassantenfrequenzStadtStgallenError) ? $result->err->msg : (string)$result->err;
