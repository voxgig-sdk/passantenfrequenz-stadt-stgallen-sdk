...
// PassantenfrequenzStadtStgallen SDK retry feature
...
class PassantenfrequenzStadtStgallenRetryFeature extends PassantenfrequenzStadtStgallenBaseFeature
{
    ...
    public function init(PassantenfrequenzStadtStgallenContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PassantenfrequenzStadtStgallenContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PassantenfrequenzStadtStgallenContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PassantenfrequenzStadtStgallenContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
