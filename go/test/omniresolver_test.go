...
import (
	...
	omni "github.com/voxgig-sdk/passantenfrequenz-stadt-stgallen-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PassantenfrequenzStadtStgallenError); is {
				out := map[string]any{
					"name":    "PassantenfrequenzStadtStgallenError",
...
func omniCtx(arg any, client *sdk.PassantenfrequenzStadtStgallenSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PassantenfrequenzStadtStgallenSDK); is {
			client = live
