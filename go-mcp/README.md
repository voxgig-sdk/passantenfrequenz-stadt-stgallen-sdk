# passantenfrequenz-stadt-stgallen-mcp

[MCP](https://modelcontextprotocol.io) server exposing the PassantenfrequenzStadtStgallen SDK as
two agent tools — `passantenfrequenz-stadt-stgallen_list` and `passantenfrequenz-stadt-stgallen_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/passantenfrequenz-stadt-stgallen-mcp)
make build

# 2. Provide credentials via the environment
export PASSANTENFREQUENZ_STADT_STGALLEN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user passantenfrequenz-stadt-stgallen \
  -- /absolute/path/to/passantenfrequenz-stadt-stgallen-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./passantenfrequenz-stadt-stgallen-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// passantenfrequenz-stadt-stgallen_list: first page of records
{ "entity": "search" }
{ "entity": "search", "query": { } }

// passantenfrequenz-stadt-stgallen_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/passantenfrequenz-stadt-stgallen-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user passantenfrequenz-stadt-stgallen \
     -- "$PWD"/dist/*/passantenfrequenz-stadt-stgallen-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `passantenfrequenz-stadt-stgallen_list` and `passantenfrequenz-stadt-stgallen_load` tools now appear
   in new sessions. Ask the agent to *"list search using passantenfrequenz-stadt-stgallen"*
   and it calls `passantenfrequenz-stadt-stgallen_list` with `{"entity":"search"}`.
...
```sh
export PASSANTENFREQUENZ_STADT_STGALLEN_APIKEY=sk_live_xxx            # API key
export PASSANTENFREQUENZ_STADT_STGALLEN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `passantenfrequenz-stadt-stgallen_list` tool
...
### Call the `passantenfrequenz-stadt-stgallen_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `passantenfrequenz-stadt-stgallen_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `passantenfrequenz-stadt-stgallen_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PASSANTENFREQUENZ_STADT_STGALLEN_APIKEY` | API key sent with every request. |
| `PASSANTENFREQUENZ_STADT_STGALLEN_BASE` | Optional override of the API base URL. |
...
```sh
./passantenfrequenz-stadt-stgallen-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"passantenfrequenz-stadt-stgallen_load","arguments":{"entity":"search","query":{"id":1}}}}'
```
