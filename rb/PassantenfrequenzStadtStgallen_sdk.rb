...
require_relative 'PassantenfrequenzStadtStgallen_types'


class PassantenfrequenzStadtStgallenSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PassantenfrequenzStadtStgallenHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PassantenfrequenzStadtStgallenHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PassantenfrequenzStadtStgallenFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PassantenfrequenzStadtStgallenUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PassantenfrequenzStadtStgallenHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PassantenfrequenzStadtStgallenHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PassantenfrequenzStadtStgallenHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => PassantenfrequenzStadtStgallenError.new(
        "#{op}_allow",
        "PassantenfrequenzStadtStgallenSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue PassantenfrequenzStadtStgallenError => err
    ...
    if fetched.is_a?(Hash)
      status = PassantenfrequenzStadtStgallenHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PassantenfrequenzStadtStgallenError.new(
        "graphql_error", "PassantenfrequenzStadtStgallenSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = PassantenfrequenzStadtStgallenSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
