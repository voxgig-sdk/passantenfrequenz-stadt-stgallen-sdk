...
module PassantenfrequenzStadtStgallenUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PassantenfrequenzStadtStgallenResponse.new({})
    ...
    if fetch_err
      ...
      response = PassantenfrequenzStadtStgallenResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PassantenfrequenzStadtStgallenResponse.new(fetched)
    else
