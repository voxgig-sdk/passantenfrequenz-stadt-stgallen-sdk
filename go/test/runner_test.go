...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("PASSANTENFREQUENZSTADTSTGALLEN_TEST_LIVE") == "TRUE" ||
		os.Getenv("PASSANTENFREQUENZSTADTSTGALLEN_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("PASSANTENFREQUENZSTADTSTGALLEN_TEST_EXPLAIN"); explain != "" {
