...
class SearchEntity
{
    ...
    {
        if ($args) {
            $this->_data = PassantenfrequenzStadtStgallenHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = PassantenfrequenzStadtStgallenHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws PassantenfrequenzStadtStgallenError on failure (item-5 convention).
     */
