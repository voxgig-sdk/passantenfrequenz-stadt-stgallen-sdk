...
// PassantenfrequenzStadtStgallen SDK feature test
//
...
class FtCtrl extends PassantenfrequenzStadtStgallenControl
{
...
class FtClient
{
    ...
    private PassantenfrequenzStadtStgallenUtility $utility;

    public function __construct(PassantenfrequenzStadtStgallenUtility $utility, array $options)
    {
    ...
    public function get_utility(): PassantenfrequenzStadtStgallenUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PassantenfrequenzStadtStgallenContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PassantenfrequenzStadtStgallenContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PassantenfrequenzStadtStgallenContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PassantenfrequenzStadtStgallenConfig::make_feature($name);
    ...
    private static function buildUrl(PassantenfrequenzStadtStgallenSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PassantenfrequenzStadtStgallenError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PassantenfrequenzStadtStgallenResponse($fetched);
        }
    ...
    private function fail(PassantenfrequenzStadtStgallenContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PassantenfrequenzStadtStgallenContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
