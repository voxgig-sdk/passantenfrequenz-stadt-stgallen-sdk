# PassantenfrequenzStadtStgallen SDK test runner
...
class PassantenfrequenzStadtStgallenTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PassantenfrequenzStadtStgallenTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PassantenfrequenzStadtStgallenTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PassantenfrequenzStadtStgallenTestRunner.getenv("PASSANTENFREQUENZ_STADT_STGALLEN_TEST_LIVE")
        override = PassantenfrequenzStadtStgallenTestRunner.getenv("PASSANTENFREQUENZ_STADT_STGALLEN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PassantenfrequenzStadtStgallenTestRunner.getenv(key)
        ...
        explain = PassantenfrequenzStadtStgallenTestRunner.getenv("PASSANTENFREQUENZ_STADT_STGALLEN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PASSANTENFREQUENZ_STADT_STGALLEN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PassantenfrequenzStadtStgallenTestRunner._test_control is not None:
            return PassantenfrequenzStadtStgallenTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PassantenfrequenzStadtStgallenTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PassantenfrequenzStadtStgallenTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PassantenfrequenzStadtStgallenTestRunner.load_test_control()
...
def load_env_local():
    PassantenfrequenzStadtStgallenTestRunner.load_env_local()


def env_override(m):
    return PassantenfrequenzStadtStgallenTestRunner.env_override(m)


def entity_data(v):
    return PassantenfrequenzStadtStgallenTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PassantenfrequenzStadtStgallenTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PassantenfrequenzStadtStgallenTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PassantenfrequenzStadtStgallenTestRunner.load_test_control()


def live_client_options():
    return PassantenfrequenzStadtStgallenTestRunner.live_client_options()


def live_delay_ms():
    return PassantenfrequenzStadtStgallenTestRunner.live_delay_ms()
