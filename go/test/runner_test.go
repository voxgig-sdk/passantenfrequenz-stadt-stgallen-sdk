...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("PASSANTENFREQUENZ_STADT_STGALLEN_TEST_LIVE") == "TRUE" ||
		os.Getenv("PASSANTENFREQUENZ_STADT_STGALLEN_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("PASSANTENFREQUENZ_STADT_STGALLEN_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.PassantenfrequenzStadtStgallenSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.PassantenfrequenzStadtStgallenSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.PassantenfrequenzStadtStgallenError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.PassantenfrequenzStadtStgallenSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.PassantenfrequenzStadtStgallenError{Msg: msg, Code: code}
}
