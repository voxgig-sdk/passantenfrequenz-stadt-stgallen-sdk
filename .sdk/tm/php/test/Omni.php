...
//    only - a typed PassantenfrequenzStadtStgallenContext there would make every ctx
...
final class PassantenfrequenzStadtStgallenOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new PassantenfrequenzStadtStgallenContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new PassantenfrequenzStadtStgallenSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new PassantenfrequenzStadtStgallenResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new PassantenfrequenzStadtStgallenError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new PassantenfrequenzStadtStgallenResponse($clean['response']);
    ...
    public static function err_from_map($m): ?PassantenfrequenzStadtStgallenError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new PassantenfrequenzStadtStgallenError(is_string($code) ? $code : '', $msg);
    }
