...
// PassantenfrequenzStadtStgallen SDK utility: result_basic

class PassantenfrequenzStadtStgallenResultBasic
{
    public static function call(PassantenfrequenzStadtStgallenContext $ctx): ?PassantenfrequenzStadtStgallenResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof PassantenfrequenzStadtStgallenError) ? $result->err->msg : (string)$result->err;
